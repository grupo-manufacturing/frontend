...
export default function ChatWindow({
  ...
  useEffect(() => {
    ...
    sentSoundRef.current.volume = 0.5;
  ...
  useEffect(() => {
    ...
    if (requirementTabs.length > 0) {
      setActiveRequirementId((current) => {
        if (current && requirementTabs.some((tab) => tab.id === current)) {
          return current;
        }
  ...
  }, [conversationId, activeRequirementId]);
  ...
  useEffect(() => {
    ...
    socket.on('message:new', ({ message }) => {
      ...
      if (messageReqId !== activeReqId) return;
  ...
  const handleSend = async () => {
    ...
    if ((!hasText && !hasFiles) || sending || !activeRequirementId) return;
    ...
    try {
      if (hasFiles) {
        ...
        const uploadPromises = selectedFiles.map((file) => apiService.uploadChatFile(file, conversationId));
      ...
      if (socketRef.current?.connected) {
        socketRef.current.emit('message:send', { conversationId, ...payload });
      } else {
        await apiService.sendMessage(conversationId, payload);
      }
    } catch (err) {
            ...
            <div className="flex items-center gap-1">
              <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-medium rounded-md transition-all bg-white text-[#22a2f2] shadow-sm"
                  title="Requirements"
                >
                  Requirements
                </button>
      ...
      {activeRequirementId && (
        <div className="border-b border-gray-200 bg-white px-4 py-2">
          {loadingRequirementDetails ? (
            ...
            (activeRequirementDetails.quantity || activeRequirementDetails.product_type || activeRequirementDetails.status) ? (
        ...
        {!loading && timelineItems.length > 0 && timelineItems.map((item) => {
          ...
          return (
            <div key={item.id} className={wrapperClass}>
              <div className={`inline-flex max-w-[80%] w-fit flex-col gap-2 whitespace-pre-wrap break-words rounded-lg px-3 py-2 text-sm shadow-sm ${bubbleTone}`}>
      ...
      <div className={inline ? 'p-3 border-t border-gray-200 bg-white' : 'p-3 border-t border-gray-200'}>
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            disabled={uploadingFiles || !activeRequirementId}
            className={inline
