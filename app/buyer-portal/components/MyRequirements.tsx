...
export default function MyRequirements({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'custom' | 'ai'>('custom');
  ...
  // Fetch AI designs when AI subtab is active
  useEffect(() => {
    if (activeSubTab === 'ai') {
      fetchAiDesigns();
    }
      ...
      <div className="mb-8 border-b-2 border-gray-200">
        <div className="flex items-center gap-1">
          <button
            ...
            className={`px-6 py-3 font-semibold text-sm transition-all relative -mb-0.5 ${
              activeSubTab === 'custom'
                ? 'text-[#22a2f2] border-b-2 border-[#22a2f2]'
